    ...
    <div>
        <header class="bg-green fg-white pb-1">
            <ul class="h-menu context h-menu-cus">
                <li>
                    <button class="button" @click="popupCreate = true"><span class="mif-add"></span>Create</button>
                </li>
            </ul>
        </header>
                            ...
                            <div class="panel-content  p-0" data-role="panel" data-width="280" data-title-caption="Recent Bills" data-title-icon="<span class='mif-apps'></span>" data-role-panel="true">
                                ...
                                <Table size="small" :columns="columns1" :data="bills">
                                    ...
                                    <template slot-scope="{ row, index }" slot="action">
                                        ...
                                        <button class="button small"  @click="preview(row._id)"><span class="mif-eye"></span></button>
                                        <button class="button small" @click="updateId = row._id; getBill()"><span class="mif-pencil"></span></button>
                                        <button class="button small" @click="destroy(row._id)"><span class="mif-bin"></span></button>
                                        </div>
                                    </template>
                                </Table>
                                <Page :current="pageCurrent" :page-size="pageSize" :total="totalCount" show-sizer @on-change="changePage" @on-page-size-change="onPageSizeChange" />
                            </div>
                            <div class="panel-title bg-light">
                                <span class="caption">Home Delivery Orders</span><img :src="billIcon" width="20"/>
                                </div>
